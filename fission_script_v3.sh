...
EOF
  ...
  ########################################
  # 5. Prometheus integration (guarded)
  ########################################
  ########################################
  # 5. Prometheus integration (FULL)
  ########################################
  if kubectl get crd servicemonitors.monitoring.coreos.com >/dev/null 2>&1; then
    echo "Prometheus Operator detected. Integrating Fission metrics..."

    ######################################
    # Label executor service
    ######################################
    kubectl label svc executor -n ${FISSION_NS} app=fission-executor --overwrite
...
EOF
    ...
    ######################################
    # Patch router service ports
    ######################################
    kubectl patch svc router -n ${FISSION_NS} --type='json' -p='[
    ...
    ######################################
    # Patch executor service ports
    ######################################
    kubectl patch svc executor -n ${FISSION_NS} --type='json' -p='[
      {"op": "add", "path": "/spec/ports/0/name", "value": "http"}
    ]' || true
    ...
    echo "✅ Prometheus + Fission integration completed"
  else
    echo "⚠️ Prometheus Operator CRDs not found — skipping metrics integration"
  fi
