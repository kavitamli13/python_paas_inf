...
force_delete_namespace() {
  local ns="$1"
  echo "Force deleting namespace $ns if stuck..."
  kubectl get ns kafka -o json > /tmp/kafka-ns.json
  sed -i 's/"finalizers": \[[^]]*\]/"finalizers": []/g' /tmp/kafka-ns.json
  kubectl replace --raw "/api/v1/namespaces/kafka/finalize" -f /tmp/kafka-ns.json

}

function uninstall_kafka() {
  echo "Uninstalling Kafka and Strimzi..."
  for t in $(kubectl get kafkatopic -n kafka -o name); do
    echo "Removing finalizers from $t"
    kubectl patch $t -n kafka --type=json -p='[{"op":"remove","path":"/metadata/finalizers"}]' || true
  done
  kubectl delete ns kafka --timeout=30s || true
  sleep 5
  force_delete_namespace kafka
}
