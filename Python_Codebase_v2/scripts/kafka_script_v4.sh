...
install_kafka() {
    ...
    # 4️⃣ Create optional Prometheus metrics ConfigMap (custom JMX rules)
...
EOF
    ...
    # 5️⃣ Create Kafka cluster (metrics removed)
...
EOF
    ...
    # 6️⃣ Create KafkaNodePools with persistent storage (no metrics field)
    cat <<EOF | kubectl apply -f -
apiVersion: kafka.strimzi.io/v1
kind: KafkaNodePool
metadata:
...
EOF
    ...
    echo "✅ Kafka cluster installed successfully. Prometheus metrics are automatically exposed."
}
...
BOOTSTRAP_NODEPORT=$(kubectl get configmap kafka-bootstrap-info -n "$KAFKA_NS" \
