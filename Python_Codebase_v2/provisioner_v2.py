...
# ---------------- APISIX Operations ----------------
async def apisix_operation(tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/apisix_create_script.sh"
    cmd = f"{script_path}"
    if tenant:
        cmd += f" {tenant}"
...
async def fission_operation(action: str, tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/fission_script_v10.sh"
...
async def postgresql_operation(action: str, tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/postgresql_script.sh"
    cmd = f"{script_path} {action}"
    if tenant:
        cmd += f" {tenant}"
...
async def rabbitmq_operation(action: str, tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/rabbitmq_script.sh"
    cmd = f"{script_path} {action}"
    if tenant:
        cmd += f" {tenant}"
...
async def redis_operation(action: str, tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/redis_script.sh"
    cmd = f"{script_path} {action}"
    if tenant:
        cmd += f" {tenant}"
...
async def provisioner(product: str, tenant: str, action: str):
    ...
    try:
        if product == "apisix":
            result = await apisix_operation(tenant)
            ...
            result = await postgresql_operation(tenant)
        elif product == "rabbitmq":
            result = await rabbitmq_operation(tenant)
        elif product == "redis":
            result = await redis_operation(tenant)
