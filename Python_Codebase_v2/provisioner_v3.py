...
# ---------------- APISIX Operations ----------------
async def apisix_operation(action: str, tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/apisix_script_v1.sh"
    ...
    log(f"Executing APISIX command: {cmd}")
...
async def fission_operation(action: str, tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/fission_script_v12.sh"
    cmd = f"{script_path} {action}"
    if tenant:
        cmd += f" {tenant}"
...
async def kafka_operation(action: str, tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/kafka_script_v1.sh"
...
async def postgresql_operation(action: str, tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/postgresql_script_v1.sh"
    cmd = f"{script_path} {action}"
    if tenant:
        cmd += f" {tenant}"
    ...
    log(f"Executing PostgreSQL command: {cmd}")
    return await exec_shell(cmd)



# ---------------- RabbitMQ Operations ----------------
async def rabbitmq_operation(action: str, tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/rabbitmq_script_v1.sh"
    cmd = f"{script_path} {action}"
    if tenant:
        cmd += f" {tenant}"
    ...
    log(f"Executing RabbitMQ command: {cmd}")
    return await exec_shell(cmd)



# ---------------- Redis Operations ----------------
async def redis_operation(action: str, tenant: str = ""):
    script_path = "/home/admusr/Python_Codebase/scripts/redis_script_v1.sh"
    cmd = f"{script_path} {action}"
    if tenant:
        cmd += f" {tenant}"
    ...
    log(f"Executing Redis command: {cmd}")
    return await exec_shell(cmd)



# ---------------- Main Provisioner ----------------
async def provisioner(product: str, tenant: str, action: str):
    ...
    try:
        if product == "apisix":
            result = await apisix_operation(action, tenant)
        elif product == "fission":
            result = await fission_operation(action, tenant)
        elif product == "kafka":
            result = await kafka_operation(action, tenant)
        elif product == "postgresql":
            result = await postgresql_operation(action, tenant)
        elif product == "rabbitmq":
            result = await rabbitmq_operation(action, tenant)
        elif product == "redis":
            result = await redis_operation(action, tenant)
...
async def provision(user_input: dict[str, Any]):
    print(user_input)
    result = await provisioner(str(user_input['product']), str(user_input['tenant']), str(user_input['action']))
    print(result)
