...
function test_function(){
  ...
  cat <<EOF > test_func.py
def main():
    return "Hello from $FUNC_NAME!"
EOF 
  fission function create --name test-func --env python-test --code test_func.py -n "$TENANT"
...
case "$1" in
  ...
  scrape-metrics)
	...
	;;
  *)
    echo "Usage: $0 {install-fission|create-tenant|delete-tenant|uninstall-fission|test-function|delete-function|scrape-metrics}"
    exit 1
    ;;
esac
