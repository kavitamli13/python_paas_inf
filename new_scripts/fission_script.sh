...
EOF
  ...
  kubectl patch svc router -n fission --type='json' -p='[{"op": "add", "path": "/spec/ports/0/name", "value": "http"}]' 
  kubectl patch svc router -n fission --type='json' -p='[ {"op": "add", "path": "/spec/ports/-", "value": {"name": "metrics", "protocol": "TCP", "port": 8080, "targetPort": 8080 }} ]' 
  kubectl patch svc executor -n fission --type='json' -p='[{"op": "add", "path": "/spec/ports/0/name", "value": "http"} ]' 
  kubectl patch svc executor -n fission --type='json' -p='[ {"op": "add", "path": "/spec/ports/-", "value": {"name": "metrics", "protocol": "TCP", "port": 8080, "targetPort": 8080 }}]'
  
  kubectl apply -f - <<EOF
apiVersion: v1
kind: Service
metadata:
  name: fission-router-metrics
  namespace: fission
  labels:
    app: fission-router
spec:
