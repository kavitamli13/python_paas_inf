...
function get_application_info() {
  ...
  local USERNAME PASSWORD
  USERNAME=$(kubectl get secret -n "$TENANT" "$AUTH_SECRET" -o jsonpath='{.data.username}' | base64 --decode)
  PASSWORD=$(kubectl get secret -n "$TENANT" "$AUTH_SECRET" -o jsonpath='{.data.password}' | base64 --decode)
...
case "$1" in
  ...
  get-application-info)
    get_application_info "$2" "$3" "$4"
    ;;
  *)
    echo "Usage: $0 {install-rabbitmq|delete-rabbitmq|get-application-info}"
    exit 1
    ;;
esac
